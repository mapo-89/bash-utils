...
center_colored_text "Die Datei 'lib.sh' wurde verschoben." "${CYAN}"
center_colored_text "Bitte verwende ab sofort:" "${CYAN}"
echo
center_colored_text "➡️  core/lib.sh" "${GREEN}"
echo
center_colored_text "Passe dein Skript wie folgt an:" "${MAGENTA}"
center_colored_text "source /usr/local/bin/bash-utils/core/lib.sh" "${CYAN}"
print_line_new "${YELLOW}"
